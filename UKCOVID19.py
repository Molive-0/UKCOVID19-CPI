    ...
    if os.path.isfile(Files["Config"]):
        ...
        if ConfigFileContents.__contains__("Configuration"):
            ...
            if not Reload:
                if Configuration.__contains__("NetworkTestAddresses"):
                    ...
                    raise Exception(
                        "Specified addresses for network test not found in file.")
            ...
            if Configuration.__contains__("TimeoutTime"):
                ...
                WriteToMainLog(
                    "Timeout time not found in configuration file. Using default timeout time.")
            if Configuration.__contains__("UKPopulation"):
                if type(Configuration["UKPopulation"]) is int:
                    ...
                    raise Exception(
                        "Key \"UKPopulation\" must be in integer data type.")
            else:
            ...
            if Configuration.__contains__("VariantsEnable"):
                ...
                WriteToMainLog(
                    "Variants toggle not found in file. Using default value.")
            if Configuration.__contains__("WaitTime"):
                DelayTime = Configuration["WaitTime"]
            else:
                WriteToMainLog(
                    "Wait time not found in file. Using default wait time.")
        ...
        if ConfigFileContents.__contains__("Files"):
            ...
            if FileList.__contains__("Messages"):
                ...
                WriteToMainLog(
                    "No messages file found in file. Defaulting to the parent folder of the script.")
            if FileList.__contains__("RollAvgPeaks"):
                Files["RollAvgPeaks"] = FileList["RollAvgPeaks"]
            else:
                raise Exception(
                    "Rolling Averages Peaks file not found in file.")
            if VariantsEnable:
                if FileList.__contains__("Variants"):
                    ...
                    VariantsEnable = False
                    WriteToMainLog(
                        "Variants file not found in file. Disabling variants function.")
        ...
        raise Exception(
            "The configuration file was not found in the specified directory.\nPlease check the file path and try again.")
    ...
    while not SuccessfulNetworkCheck:
        try:
            for i in range(len(NetworkTestAddresses)):
                WriteToMainLog("Testing connection to IP address " +
                               NetworkTestAddresses[i] + ". . .")
                R = requests.get(NetworkTestAddresses[i])
                if R.status_code != 200 and R.status_code != 204:
                    raise Exception("Request fail with status code " +
                                    str(R.status_code) + " on address " + NetworkTestAddresses[i])
                WriteToMainLog(
                    "Test completed with status code " + str(R.status_code) + ".")
                time.sleep(2)
...
async def TimeReview():
    try:
        global DataAggregationTemplate, Files, LatestRecordFormatted, PrimaryUpdated, SecondaryUpdated, StatusMessagesAddresses
        ...
        while True:
            ...
            if DateOfCurrentData == CurrentDate and not PrimaryUpdated:
                ...
                if CurrentTime >= BeginTime and not (PrimaryUpdated or SecondaryUpdated):
                    ...
                    LatestRecordFormatted = loads(
                        dumps(DataAggregationTemplate))
                    await APICheck()
...
async def APICheck():
    global DateOfCurrentData, DelayTime, LatestRecordFormatted, PrimaryUpdated, SecondaryUpdated
    await WaitForDiscord()
...
def ReloadMassData(CalculateRollAvgPeak=True):
    ...
    for i in range(len(DataFromAPI)):
        ...
        CurrentRecordFormatting["Day"] = datetime.strptime(
            DataFromAPI[i]["Date"], "%Y-%m-%d").weekday()
        ...
        if DataFromAPI[i]["CasesTotal"] != None and DataFromAPI[i]["DeathsTotal"] != None:
            CurrentRecordFormatting["CaseFatality"]["Rate"] = DataFromAPI[i]["DeathsTotal"] / \
                DataFromAPI[i]["CasesTotal"]
        AllData.append(CurrentRecordFormatting)
    WriteToMainLog(
        "Mass data formatted. Calculating rolling averages & daily change. . .")
        ...
        for Metric in Metrics:
            ...
            if i != 0:
                if AllData[i - 1][Metric]["New"] != None and AllData[i][Metric]["New"] != None:
                    AllData[i - 1][Metric]["Change"] = AllData[i -
                                                               1][Metric]["New"] - AllData[i][Metric]["New"]
                    AllData[i - 1][Metric]["Corrections"] = AllData[i - 1][Metric]["Total"] - \
                        (AllData[i - 1][Metric]["New"] +
                         AllData[i][Metric]["Total"])
                if AllData[i - 1]["CaseFatality"]["Rate"] != None and AllData[i]["CaseFatality"]["Rate"] != None:
                    AllData[i - 1]["CaseFatality"]["Change"] = AllData[i -
                                                                       1]["CaseFatality"]["Rate"] - AllData[i]["CaseFatality"]["Rate"]
                ...
                for RollingAverage in RollingAverages:
                    if AllData[i][Metric]["RollingAverages"][RollingAverage]["Average"] != None:
                        AllData[i - 1][Metric]["RollingAverages"][RollingAverage]["Change"] = AllData[i -
                                                                                                      1][Metric]["RollingAverages"][RollingAverage]["Average"] - AllData[i][Metric]["RollingAverages"][RollingAverage]["Average"]
    return AllData
        ...
        for Metric in Metrics:
            ...
            if RollingAverage != None:
                ...
                if not RollingAveragePeak and RollAvgPeaks[Metric]["Local"]["Date"] != None:
                    if AllData[i][Metric]["RollingAverages"]["Seven"]["Change"] < 0:
                        DateOfLastLocal = datetime.strptime(
                            RollAvgPeaks[Metric]["Local"]["Date"], "%Y-%m-%d")
                        DateOfCurrentData = datetime.strptime(
                            AllData[i]["Date"], "%Y-%m-%d")
...
def CommitToFile(AllData, RollAvgPeaks):
    ...
    if RollAvgPeaks != None:
        ...
        with open(Files["RollAvgPeaks"], 'w') as RollAvgPeaksFile:
            ...
            RollAvgPeaksFile.write(
                "  \"Cases\": " + dumps(RollAvgPeaks["Cases"]) + ",\n")
            RollAvgPeaksFile.write(
                "  \"Deaths\": " + dumps(RollAvgPeaks["Deaths"]) + "\n")
            RollAvgPeaksFile.write("}")
...
def ParseData(Data):
    ...
    LatestRecordFormatted["Day"] = datetime.strptime(
        Data["Date"], "%Y-%m-%d").weekday()
    for Metric in Metrics:
        LatestRecordFormatted[Metric]["New"] = Data[Metric + "New"]
        LatestRecordFormatted[Metric]["Total"] = Data[Metric + "Total"]
        LatestRecordFormatted["CaseFatality"]["Rate"] = Data["DeathsTotal"] / \
            Data["CasesTotal"]
        if AllData[0]["CaseFatality"]["Rate"] != None:
            LatestRecordFormatted["CaseFatality"]["Change"] = LatestRecordFormatted["CaseFatality"]["Rate"] - \
                AllData[0]["CaseFatality"]["Rate"]
        if AllData[0][Metric]["New"] != None:
            LatestRecordFormatted[Metric]["Change"] = Data[Metric +
                                                           "New"] - AllData[0][Metric]["New"]
            if AllData[0][Metric]["Total"] != None:
                LatestRecordFormatted[Metric]["Corrections"] = Data[Metric +
                                                                    "Total"] - (Data[Metric + "New"] + AllData[0][Metric]["Total"])
...
def CalculateRollingAverages(NumOfDays, AllData, NewData):
    global LatestRecordFormatted
    WriteToMainLog("Calculating rolling averages of length " +
                   str(NumOfDays) + ". . .")
    if NumOfDays == 3:
        RollingAverageLength = "Three"
    else:
    ...
    for Metric in Metrics:
        ...
        if RollingAverage != None:
            ...
            if AllData[0][Metric]["RollingAverages"][RollingAverageLength]["Average"] != None:
                LatestRecordFormatted[Metric]["RollingAverages"][RollingAverageLength]["Change"] = RollingAverage - \
                    AllData[0][Metric]["RollingAverages"][RollingAverageLength]["Average"]
...
async def CheckRollAvgPeaks():
    ...
    if not VerifyMassData(ReloadIfFail=False):
        ...
        for Metric in Metrics:
            RollAvgPeaks[Metric] = LookForPeak(
                Metric.upper(), RollAvgPeaks[Metric], CurrentPeaks)
        ...
        if len(Output) != 0:
            FinalOutput = ""
            for Line in Output:
                FinalOutput += Line.replace("%CASESPLACEHOLDER%", CasesRAPlaceholder).replace(
                    "%DEATHSPLACEHOLDER%", DeathsRAPlaceholder) + "\n"
...
def LookForPeak(Metric, Flags, CurrentPeaks):
    ...
    if not Flags["NewLocal"] and CurrentPeaks[Metric]["Local"]["Value"] != None:
        if LatestRecordFormatted[Metric]["RollingAverages"]["Seven"]["Change"] < 0:
            DateOfLastLocal = datetime.strptime(
                CurrentPeaks[Metric]["Local"]["Date"], "%Y-%m-%d")
            CurrentDate = datetime.strptime(
                LatestRecordFormatted["Date"], "%Y-%m-%d")
...
def CommitPeaksToFile(Flags, CurrentPeaks):
    WriteToMainLog(
        "Checking if Rolling Average Peaks file needs recommitting. . .")
    ChangesMade = True
    for Metric in Metrics:
        ...
        for i in range(StartingIndex, len(AllData)):
            ...
            if type(CurrentIndex[Metric]["New"]) is int:
                if CurrentIndex[Metric]["New"] > CheckData[Metric]["New"]:
                    LastHighestDate = CurrentIndex["Date"] + \
                        "; {:,}".format(CurrentIndex[Metric]["New"])
                    break
    else:
...
def ShowRollAvgPeaks(RequestedMetric=None, RequestedLength=None, ShowHeadings=False):
    ...
    if RequestedMetric == None:
        ...
        for Length in Lengths:
            Output += "\n    " + Length + ":" + \
                ShowRollAvgPeaks(RequestedMetric.upper(), Length.upper())
    else:
        RequestedMetric = RequestedMetric[0] + \
            RequestedMetric[1:len(RequestedMetric)].lower()
        RequestedLength = RequestedLength[0] + \
            RequestedLength[1:len(RequestedLength)].lower()
        ...
        if type(RollAvgPeaks[RequestedMetric][RequestedLength]["Value"]) is float:
            Output += "\n      Average: {:,}".format(
                round(RollAvgPeaks[RequestedMetric][RequestedLength]["Value"], 3))
        else:
            Output += "\n      Average: None"
        Output += "\n      Date:    " + \
            str(RollAvgPeaks[RequestedMetric][RequestedLength]["Date"])
    return Output
...
async def SendData(Structure, Data, Index=0):
    await WaitForDiscord()
    WriteToMainLog("Building Discord message for " +
    ...
    if Structure == "PRIMARY":
        ...
        Output += "PRIMARY DATA FOR " + \
            Data["Date"] + ", " + Weekdays[Data["Day"]]
        for Metric in Metrics:
            Output += "\n" + Emoji[Metric] + Metric + ":"
            ...
            if type(Data[Metric]["Change"]) is int:
                Output += "\n    Change:       {:,}".format(
                    Data[Metric]["Change"]) + GetArrow(Data[Metric]["Change"])
            else:
                Output += "\n    Change:       None" + Emoji["Cross"]
            if ShowLastHighest:
                Output += "\n    Last Highest: " + \
                    FindLastHighest(AllData, Data, Metric, Index)
            else:
                Output += "\n    Last Highest: None"
            for RollingAverage in RollingAverages:
                ...
                if type(Data[Metric]["RollingAverages"][AverageWord]["Average"]) is float:
                    Output += "\n      Average:    {:,}".format(round(
                        Data[Metric]["RollingAverages"][AverageWord]["Average"], NumDecimalPointForRounding))
                else:
                    Output += "\n      Average:    None"
                if type(Data[Metric]["RollingAverages"][AverageWord]["Change"]) is float:
                    Output += "\n      Change:     {:,}".format(round(Data[Metric]["RollingAverages"][AverageWord]["Change"], NumDecimalPointForRounding)) + GetArrow(
                        Data[Metric]["RollingAverages"][AverageWord]["Change"])
                else:
                    Output += "\n      Change:     None" + Emoji["Cross"]
            if type(Data[Metric]["Corrections"]) is int:
                Output += "\n    Corrections:  {:,}".format(
                    Data[Metric]["Corrections"])
            else:
                Output += "\n    Corrections:  None"
        ...
        if type(Data["CaseFatality"]["Rate"]) is float:
            Output += "\n    Rate:         {:,}%".format(
                round(Data["CaseFatality"]["Rate"] * 100, NumDecimalPointForRounding))
        else:
            Output += "\n    Rate:         None"
        if type(Data["CaseFatality"]["Change"]) is float:
            Output += "\n    Change:       {:,}p.p.".format(
                round(Data["CaseFatality"]["Change"] * 100, NumDecimalPointForRounding))
        else:
            Output += "\n    Change:       None" + Emoji["Cross"]
        ...
        Output += Emoji["Secondary"] + "SECONDARY DATA FOR " + Data["Date"] + \
            ", " + \
            Weekdays[datetime.strptime(Data["Date"], "%Y-%m-%d").weekday()]
        ...
        for Dose in VaccinationDoses:
            ...
            Output += "\n      New:          {:,}".format(
                Data["Vaccinations" + Dose + "DoseNew"])
            Output += "\n      Total:        {:,}".format(
                Data["Vaccinations" + Dose + "DoseTotal"])
            Output += "\n      % Population: {:,}%".format(round(
                (Data["Vaccinations" + Dose + "DoseTotal"] / UKPopulation) * 100, NumDecimalPointForRounding))
        Output += "\n  Vaccinations (Total Doses):"
        Output += "\n      New:          {:,}".format(TotalDoses["New"])
        Output += "\n      Total:        {:,}".format(TotalDoses["Total"])
        Output += "\n      % Population: {:,}%".format(
            round((TotalDoses["Total"] / UKPopulation) * 100, NumDecimalPointForRounding))
    Output += "\n```"
    WriteToMainLog("Sending Discord message for " +
                   Structure.lower() + " structure. . .")
        ...
        if Message.channel == DiscordClient.get_channel(id=ChannelID) and len(Message.content) > 0:
            if Message.content[0] == "$":
                ...
                if Command.upper().startswith("$GETDATA"):
                    ...
                    WriteToMainLog(
                        "Command received of unknown type. Returning command help.")
                    await CommandHelp()
    ...
    if len(AllData) == 0:
        ...
        if len(Command) == 2:
            if VerifyDate(Command[1]):
                WriteToMainLog("Data requested for " +
                               Command[1] + ". Obtaining data. . .")
                DataFound = False
                for i in range(len(AllData)):
...
async def RollAvgPeaksCommand(Command):
    ...
    if len(Command) == 1:
        ...
        if list(map(lambda x: x.upper(), Metrics)).__contains__(Command[1].upper()):
            Output = "```\nRolling Average Peaks (7-Day):\n  " + Command[1][0].upper() + Command[1][1:len(
                Command[1])].lower() + ":" + ShowRollAvgPeaks(Command[1].upper()) + ClosingText + "\n```"
        ...
        if list(map(lambda x: x.upper(), Metrics)).__contains__(Command[1].upper()) and ["LOCAL", "GLOBAL"].__contains__(Command[2].upper()):
            Output = "```\nRolling Average Peaks (7-Day):" + ShowRollAvgPeaks(
                Command[1].upper(), Command[2].upper(), True) + ClosingText + "\n```"
        else:
...
async def VersionCommand():
    ...
    Output = "COVID Pi and ~~UK-COV19 Bot~~ Botty-Mc-Bot-Face Version " + \
        Version + ".\nChangelog:\n```"
...
async def CheckForMessage(CurrentDate=None):
    try:
        ...
        while not SuccessfulCheck:
            try:
                WriteToMainLog(
                    "Checking for announcement (yellow) messages. . .")
                Messages = loads(requests.get(
                    StatusMessagesAddresses["YellowBannersAddress"]).text)
                for Message in Messages:
                    if Message["date"] == CurrentDate:
                        if not MessageAlreadySent(Message["body"], ExistingMessages, Message["date"]):
                            await SendMessage(CurrentDate, Message["body"], "Dashboard")
...
def WriteToMainLog(Text, Date=True):
    if Date:
        Output = "[" + datetime.now().astimezone().replace(
            microsecond=0).isoformat(sep='T') + "] " + Text + "\n"
    else:
        Output = Text + "\n"
...
def PrintError():
    with open(Files["RuntimeLogs"].replace("%DATE%", date.today().isoformat()), 'a') as LogFile:
        LogFile.write("["+datetime.now().astimezone().replace(
            microsecond=0).isoformat(sep='T')+"] Critical Error {\n")
...
async def FatalException(WriteToFile=True):
    if WriteToFile:
        ErrorLogFilename = Files["ErrorLogs"] + "Error_" + \
            datetime.now().strftime("%Y-%m-%dT%H%M%S") + ".txt"
        with open(ErrorLogFilename, 'a') as ErrorFile:
            ErrorFile.write("[" + datetime.now().astimezone().replace(
                microsecond=0).isoformat(sep='T') + "] Fatal Error (Exception point 1) {\n")
...
if __name__ == "__main__":
    try:
        ...
        with open(ErrorLogFilename, 'a') as ErrorFile:
            ErrorFile.write("[" + datetime.now().astimezone().replace(
                microsecond=0).isoformat(sep='T') + "] Fatal Error (Exception point 2) {\n")
            ErrorFile.write(traceback.format_exc())
            ErrorFile.write("\n}\n")
